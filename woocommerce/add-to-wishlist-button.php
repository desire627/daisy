...
<div class="yith-wcwl-add-button">
	<a href="<?php echo esc_url( add_query_arg( 'add_to_wishlist', $product_id, $base_url ) ); ?>"
		rel="nofollow" data-product-id="<?php echo esc_attr( $product_id ) ?>"
		data-product-type="<?php echo esc_attr( $product_type ); ?>"
		data-original-product-id="<?php echo esc_attr( $parent_product_id ); ?>" class="tz-loop-button <?php echo esc_attr( $link_classes ); ?>"
		data-title="<?php echo esc_attr( apply_filters( 'yith_wcwl_add_to_wishlist_title', $label ) ); ?>"
		data-text="<?php echo esc_attr( apply_filters( 'yith_wcwl_add_to_wishlist_title', $label ) ); ?>"
		data-product-title="<?php echo esc_attr( $product_title ); ?>">
		<?php echo ! empty( $icon ) ? wp_kses($icon,'post') : ''; ?>
		<span class="loop_button-text"><?php echo wp_kses( $label,'post' ); ?></span>
	</a>
</div>