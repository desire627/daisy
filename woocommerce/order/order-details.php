...
$item_text = esc_html__('Order items', 'travelami');

if ( $item_count == 1 ) {
	$item_text = esc_html__('Order item', 'travelami');
}
?>
		...
		<thead>
			<tr>
				<th class="woocommerce-orders-table__cell order-number">
					...
					<span><?php echo esc_html( _x( '#', 'hash before order number', 'travelami' ) . $order->get_order_number() ); ?></span>
				</th>
				...
				<th class="woocommerce-orders-table__cell order-total">
					...
					<span><?php echo wp_kses( sprintf( _n( '%1$s', '%1$s', $item_count, 'travelami' ), $order->get_formatted_order_total() ),'post' ); ?></span>
				</th>
				<th class="woocommerce-orders-table__cell order-method">
					<div class="order-title"><?php esc_html_e('Payment method:', 'travelami') ?></div>
					<span><?php echo wp_kses( $order->get_payment_method_title(),'post' ); ?></span>
				</th>
			</tr>
		</thead>
...
<section class="woocommerce-order-total">
	<h2 class="woocommerce-order-downloads__title"><?php esc_html_e('Order Total', 'travelami') ?></h2>
	...
	<table class="woocommerce-table woocommerce-table--order-details shop_table order_details order_total">
		<tbody>
			...
			foreach ( $order->get_order_item_totals() as $key => $total ) {
				?>
					<tr>
						...
						<td><?php echo ( 'payment_method' === $key ) ? esc_html( $total['value'] ) : wp_kses( $total['value'],'post' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></td>
					</tr>
					<?php
			}
			?>
			<?php if ( $order->get_customer_note() ) : ?>
				<tr>
					<th><?php esc_html_e( 'Note:', 'travelami' ); ?></th>
					<td><?php echo wp_kses( nl2br( wptexturize( $order->get_customer_note() ) ),'post' ); ?></td>
				</tr>
			<?php endif; ?>
		</tbody>
	</table>
