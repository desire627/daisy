...
class Templaza_Woo_General {
	...
	public function star_rating_html( $html, $rating, $count ) {
		...
		if ( 0 < $count ) {
			...
			$html .= sprintf( _n( 'Rated %1$s out of 5 based on %2$s customer rating', 'Rated %1$s out of 5 based on %2$s customer ratings', $count, 'travelami' ), '<strong class="rating">' . esc_html( $rating ) . '</strong>', '<span class="rating">' . esc_html( $count ) . '</span>' );
		} else {
			/* translators: %s: rating */
			$html .= sprintf( esc_html__( 'Rated %s out of 5', 'travelami' ), '<strong class="rating">' . esc_html( $rating ) . '</strong>' );
		}
	...
	public function get_product_availability_text( $availability, $product ) {
		...
		if ( ! $product->managing_stock() && $product->get_stock_status() == 'instock' ) {
			$availability = esc_html__( 'In stock', 'travelami' );
		}
