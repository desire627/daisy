...
class Templaza_Woo_Quick_View {
	...
	public function templaza_quick_view() {
		if ( empty( $_POST['product_id'] ) ) {
			wp_send_json_error( esc_html__( 'No product.', 'travelami' ) );
			exit;
		}
		...
		if ( ! $post_object || ! in_array( $post_object->post_type, array(
			...
			wp_send_json_error( esc_html__( 'Invalid product.', 'travelami' ) );
			exit;
		}
	...
	public function templaza_product_quick_view_more_info_button() {
		printf(
			...
			apply_filters( 'product_quick_view_more_infor_text', esc_html__( 'More Product Info', 'travelami' ) ),
