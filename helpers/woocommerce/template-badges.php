...
class Templaza_Woo_Badges {
	...
	public function get_sale_flash( $output, $post, $product ) {
		...
		if ( 'percent' == $type ) {
			...
			$output = '<span class="onsale woocommerce-badge"><span class="text">' . wp_kses( $text,'post' ) . '</span><span class="percent"> ' . $percentage . '%</span></span>';
		} else {
			$output = '<span class="onsale woocommerce-badge"><span>' . wp_kses( $text,'post' ) . '</span></span>';
		}
