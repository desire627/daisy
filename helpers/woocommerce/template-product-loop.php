...
class Templaza_Product_Loop {
	...
	public function product_loop_content_hooks() {
        ...
        $attributes       = isset($templaza_options['templaza-shop-loop-attributes'])?$templaza_options['templaza-shop-loop-attributes']:array('taxonomy'=>'1','rating'=>'1');
		// Remove wrapper link
		...
		add_filter( 'travelami_tz_wp_script_data', array( $this, 'loop_script_data' ) );
	...
	public function product_loop_layout() {
        ...
        $featured_icons       = isset($templaza_options['templaza-shop-loop-featured-icons'])?$templaza_options['templaza-shop-loop-featured-icons']:array('cart'=>'1','quickview'=>'1','wishlist'=>'1',);
	...
	public function product_loop_thumbnail() {
		...
		switch ( $product_hover ) {
			...
			case 'fadein':
				...
				if ( ! empty( $image_ids ) ) {
					$image_size = apply_filters( 'single_product_archive_thumbnail_size', 'woocommerce_thumbnail' );
					echo wp_kses(wp_get_attachment_image( $image_ids[0], $image_size, false, array( 'class' => 'attachment-woocommerce_thumbnail size-woocommerce_thumbnail hover-image' ) ),'post');
				}
			...
			case 'zoom';
				...
				break;
			default:
				echo '<div class="product-thumbnail 2222">';
				woocommerce_template_loop_product_link_open();
				woocommerce_template_loop_product_thumbnail();
				woocommerce_template_loop_product_link_close();
				do_action( 'templaza_product_loop_thumbnail' );
	...
	public function product_loop_quick_shop() {
		...
		echo sprintf(
			...
			esc_html__( 'Quick Shop', 'travelami' )
		);
	}
	...
	public function product_variable_add_to_cart_text( $text ) {
		...
		if ( $product->is_purchasable() ) {
			$text = esc_html__( 'Add to cart', 'travelami' );
