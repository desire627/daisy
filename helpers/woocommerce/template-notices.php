...
class Templaza_Woo_Notices {
    ...
    public function __construct() {
        ...
        add_filter( 'travelami_tz_wp_script_data', array(
            $this,
            'notices_script_data'
        ), 30 );
   }
        ...
        <div id="templaza-popup-add-to-cart" class="templaza-popup-add-to-cart templaza-modal woocommerce">
            ...
            <div class="modal-content container woocommerce">
                ...
                <div class="templaza-product-popup-atc__notice">
                    <?php esc_html_e( 'Successfully added to your cart.', 'travelami' ) ?>
                </div>
            ...
            foreach ( $query_posts as $product_id ) {
                ...
                <li class="product-item">
                    <a href="<?php echo esc_url( $_product->get_permalink() ); ?>">
                        <?php echo wp_kses( $_product->get_image( 'woocommerce_thumbnail' ),'post' ); ?>
                        <span class="product-title"><?php echo wp_kses( $_product->get_name(),'post' ); ?></span>
                        <span class="product-price"><?php echo wp_kses( $_product->get_price_html(),'post' ); ?></span>
                    </a>
                </li>
    ...
    */
    public function notices_script_data( $data ) {
        ...
        if ( $cart_notify == 'simple' ) {
            $data['added_to_cart_notice'] = array(
                'added_to_cart_text'              => esc_html__( 'has been added to your cart.', 'travelami' ),
                'successfully_added_to_cart_text' => esc_html__( 'Successfully added to your cart.', 'travelami' ),
                'cart_view_text'                  => esc_html__( 'View Cart', 'travelami' ),
        ...
        if ( $wishlist_notify && defined( 'YITH_WCWL' ) ) {
            $data['added_to_wishlist_notice'] = array(
                'added_to_wishlist_text'    => esc_html__( 'has been added to your wishlist.', 'travelami' ),
                'added_to_wishlist_texts'   => esc_html__( 'have been added to your wishlist.', 'travelami' ),
                'wishlist_view_text'        => esc_html__( 'View Wishlist', 'travelami' ),
	...
	public function widget_shopping_cart_count_notice() {
		...
		echo sprintf( 'There %s %s %s in your cart', _n( 'are', 'is', $count > 1, 'travelami' ), $count, _n( 'items', 'item', $count > 1, 'travelami' ) );
		echo '</span>';
	}
}
