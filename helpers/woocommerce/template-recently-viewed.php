...
class Templaza_Woo_Recently_Viewed {
	...
	public function products_content() {
		...
		if ( empty( $this->product_ids ) ) {
			printf(
				...
				'</ul>',
				esc_html__( 'Recently Viewed Products is a function which helps you keep track of your recent viewing history.', 'travelami' ),
				esc_url( get_permalink( get_option( 'woocommerce_shop_page_id' ) ) ),
				esc_html__( 'Shop Now', 'travelami' )
			);
		} else {
	...
	public function products_recently_viewed_section() {
        ...
        $recent_viewed      = isset($templaza_options['templaza-shop-recent-viewed'])?filter_var($templaza_options['templaza-shop-recent-viewed'], FILTER_VALIDATE_BOOLEAN):false;
        $display_page       = isset($templaza_options['templaza-shop-recent-viewed-page'])?$templaza_options['templaza-shop-recent-viewed-page']:array('single'=>'1','catalog'=>'','cart'=>'','checkout'=>'',);
        $recent_viewed_ajax      = isset($templaza_options['templaza-shop-recent-viewed-ajax'])?filter_var($templaza_options['templaza-shop-recent-viewed-ajax'], FILTER_VALIDATE_BOOLEAN):true;
