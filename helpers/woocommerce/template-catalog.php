...
class Templaza_Woo_Catalog {
	...
	public function scripts() {
		...
		wp_enqueue_script( 'templaza-product-catalog', get_template_directory_uri() . '/assets/js/woo/woo-catalog.js', array(
			'travelami',
		), '20211209', true );

		$templaza_catalog_data = array(
			'filtered_price' => array(
				'min' => esc_html__( 'Min', 'travelami' ),
				'max' => esc_html__( 'Max', 'travelami' ),
			)
		);
	...
	public function shop_content_open_wrapper() {
        if ( !class_exists( 'TemPlazaFramework\TemPlazaFramework' )){
            echo '<div class="templaza-content-session uk-container uk-container-large">';
        }
	...
	public function pagination() {
		...
		if ( 'number' == $pagination_type ) {
			...
			if ( get_next_posts_link() ) {
				...
				$nav_html = sprintf( '<span class="button-text">%s</span>', esc_html__( 'Load More', 'travelami' ) );
        ...
        <div id="catalog-filters-modal" class="catalog-filters-modal templaza-modal" tabindex="-1" role="dialog">
            ...
            <div class="filters-panel-content panel-content">
                <div class="modal-header">
                    <h3 class="modal-title"><?php esc_html_e( 'Filter By', 'travelami' ) ?></h3>
                    <a href="#"
	...
	public function products_tabs() {
		...
		if ( 'category' == $type ) {
			...
			$labels = array(
				'best_sellers' => esc_html__( 'Best Sellers', 'travelami' ),
				'featured'     => esc_html__( 'Hot Products', 'travelami' ),
				'new'          => esc_html__( 'New Products', 'travelami' ),
				'sale'         => esc_html__( 'Sale Products', 'travelami' ),
			);
		...
		array_unshift( $tabs, sprintf(
			...
			esc_url( $btn_url ),
			$active ? '' : 'active',
			esc_html__( 'All', 'travelami' )
		) );

		$text_toggle = $type == 'group' ? esc_html__( 'Products', 'travelami' ) : esc_html__( 'Categories', 'travelami' );
	...
	public function products_filter_button() {
        ...
        <a href="#catalog-filters" class="toggle-filters catalog-toolbar-item__control"
           ...
           data-target="catalog-filters-<?php echo esc_attr( $open ) ?>">
            <span class="text-filter"><?php esc_html_e( 'Filter', 'travelami' ) ?></span>
        </a>
		<?php
	}
	...
	public function products_ordering() {
		$catalog_orderby_options = apply_filters( 'templaza_products_filter_order_by', array(
			'menu_order' => esc_html__( 'Default sorting', 'travelami' ),
			'popularity' => esc_html__( 'Sort by popularity', 'travelami' ),
			'rating'     => esc_html__( 'Sort by average rating', 'travelami' ),
			'date'       => esc_html__( 'Sort by latest', 'travelami' ),
			'price'      => esc_html__( 'Sort by price: low to high', 'travelami' ),
			'price-desc' => esc_html__( 'Sort by price: high to low', 'travelami' ),
		) );
        ...
        <div class="woocommerce-ordering">
            <span class="woocommerce-ordering__button"><span
                        class="woocommerce-ordering__button-label"><?php echo ! empty( $orderby ) ? esc_html($order_current) : esc_html__( 'Default', 'travelami' ) ?> </span> <i class="far fa-chevron-down"></i></span>

            <ul class="woocommerce-ordering__submenu">
				<?php echo wp_kses( $order_html,'post' ); ?>
            </ul>
