(function ($) {
	...
	templaza_woo.instanceSearch = function () {
		if (travelamiTzData.header_ajax_search != '1') {
			return;
		}
		...
		function search($currentForm) {
			...
			if (keycat in searchCache) {
				...
				var data = {
						'term': keyword,
						'cat': cat,
						'ajax_search_number': travelamiTzData.header_search_number,
						'search_type': travelamiTzData.search_content_type
					},
					ajax_url = travelamiTzData.ajax_url.toString().replace('%%endpoint%%', 'templaza_instance_search_form');
	...
	templaza_woo.updateQuantityAuto = function() {
		$( document.body ).on( 'change', 'table.cart .qty', function() {
			if (typeof travelamiTzData.update_cart_page_auto !== undefined && travelamiTzData.update_cart_page_auto == '1') {
	...
	templaza_woo.crossSellProductsCarousel = function () {
		...
		new Swiper($crossSells.find('.linked-products-carousel'), {
			...
			breakpoints: {
				300: {
					slidesPerView: travelamiTzData.mobile_portrait,
					slidesPerGroup: travelamiTzData.mobile_portrait,
					spaceBetween: 20,
				},
				480: {
					slidesPerView: travelamiTzData.mobile_landscape,
					slidesPerGroup: travelamiTzData.mobile_landscape,
				},
				768: {
					spaceBetween: 20,
					slidesPerView: 3,
	...
	templaza_woo.productLoopHover = function () {
		if (typeof travelamiTzData.product_loop_layout === 'undefined') {
			return;
		}

		if (travelamiTzData.product_loop_layout !== 'layout-8') {
			return;
		}
	...
	templaza_woo.productLoopFormAJAX = function () {
		...
		if (typeof travelamiTzData.product_loop_variation_ajax === 'undefined') {
			return;
		}

		if (travelamiTzData.product_loop_layout !== '9') {
			return;
		}

		if (travelamiTzData.product_loop_variation_ajax !== '1') {
			return;
		}
		...
		templaza_woo.$body.on('click', '.product-quick-shop-button', function (e) {
			...
			$.ajax({
				url: travelamiTzData.ajax_url.toString().replace('%%endpoint%%', 'templaza_product_loop_form'),
				type: 'POST',
				data: {
					nonce: travelamiTzData.nonce,
	...
	templaza_woo.productLoopATCForm = function () {
		if (typeof travelamiTzData.product_loop_variation === 'undefined') {
			return;
		}

		if (travelamiTzData.product_loop_variation !== '1') {
			return;
		}
	...
	templaza_woo.productLoopHoverZoom = function () {
		if (typeof travelamiTzData.product_loop_hover === 'undefined' || !$.fn.zoom) {
			return;
		}

		if (travelamiTzData.product_loop_hover !== 'zoom') {
			return;
		}
	...
	templaza_woo.productQuickView = function () {
		$(document.body).on('click', '.quick-view-button', function (event) {
			...
			var $el = $(this),
				...
				ajax_url = travelamiTzData.ajax_url.toString().replace('%%endpoint%%', 'product_quick_view');
	...
	templaza_woo.loginPanelAuthenticate = function () {
		$('#account-modal').on('submit', 'form.login', function authenticate(event) {
			var username = $('input[name=username]', this).val(),
				...
				remtravelami = $('input[name=rememberme]', this).is(':checked'),
			...
			var ajax_url = travelamiTzData.ajax_url.toString().replace('%%endpoint%%', 'templaza_login_authenticate');
	...
	templaza_woo.openMiniCartPanel = function () {
		if (typeof travelamiTzData.added_to_cart_notice === 'undefined') {
			return;
		}

		if (travelamiTzData.added_to_cart_notice.added_to_cart_notice_layout !== 'panel') {
			return;
		}
	...
	templaza_woo.addedToCartNotice = function () {
		...
		if (typeof emberTzData.added_to_cart_notice === 'undefined' || !$.fn.notify) {
			return;
		}

		if (emberTzData.added_to_cart_notice.added_to_cart_notice_layout != 'simple') {
			return;
		}
		...
		function getaddedToCartNotice($content) {
			...
			if ($content) {
				$content += ' ' + emberTzData.added_to_cart_notice.added_to_cart_text;
			} else {
				$content = emberTzData.added_to_cart_notice.successfully_added_to_cart_text;
			}

			$content += '<a href="' + emberTzData.added_to_cart_notice.cart_view_link + '" class="btn-button">' + emberTzData.added_to_cart_notice.cart_view_text + '</a>';
			...
			$.notify('&nbsp', {
				autoHideDelay: emberTzData.added_to_cart_notice.cart_notice_auto_hide,
				className: 'success',
				style: 'templaza',
				showAnimation: 'fadeIn',
				hideAnimation: 'fadeOut'
	...
	templaza_woo.productPopupATC = function () {
		...
		if (typeof emberTzData.added_to_cart_notice === 'undefined') {
			return;
		}

		if (emberTzData.added_to_cart_notice.added_to_cart_notice_layout != 'popup') {
			return;
		}
		...
		function getProductPopupContent($product_id, $product_item_id) {
			...
			if (!$recomended.hasClass('loaded')) {
				...
				$.ajax({
					url: emberTzData.ajax_url.toString().replace('%%endpoint%%', 'templaza_product_popup_recommended'),
					type: 'POST',
					data: {
						nonce: emberTzData.nonce,
						product_id: $product_id
					},
					success: function (response) {
						if (!response || response.data === '') {
		...
		function productsCarousel($selector) {
			...
			new Swiper($selector.find('.recommendation-products-carousel'), {
				...
				breakpoints: {
					300: {
						slidesPerView: emberTzData.mobile_portrait,
						slidesPerGroup: emberTzData.mobile_portrait,
						spaceBetween: 20,
					},
					480: {
						slidesPerView: emberTzData.mobile_landscape,
						slidesPerGroup: emberTzData.mobile_landscape,
					},
					768: {
						spaceBetween: 20,
						slidesPerView: 3,
	...
	templaza_woo.addedToWishlistNotice = function () {
		if (typeof emberTzData.added_to_wishlist_notice === 'undefined' || !$.fn.notify) {
			return;
		}
		...
		function getaddedToWishlistNotice($content) {
			...
			$content += ' ' + emberTzData.added_to_wishlist_notice.added_to_wishlist_text;

			$content += '<a href="' + emberTzData.added_to_wishlist_notice.wishlist_view_link + '" class="btn-button">' + emberTzData.added_to_wishlist_notice.wishlist_view_text + '</a>';
			...
			$.notify('&nbsp', {
				autoHideDelay: emberTzData.added_to_wishlist_notice.wishlist_notice_auto_hide,
				className: 'success',
				style: 'templaza',
				showAnimation: 'fadeIn',
				hideAnimation: 'fadeOut'
	...
	templaza_woo.newsletterPopup = function () {
		if (!emberTzData.popup) {
			return;
		}
		var $modal = $('#newsletter-popup-modal'),
			days = parseInt(emberTzData.popup_frequency),
			delay = parseInt(emberTzData.popup_visible_delay);
		...
		delay = 'delay' === emberTzData.popup_visible ? delay : 0;
	...
	templaza_woo.recentlyViewedProducts = function () {
		...
		function loadProductsAJAX() {
			...
			var ajax_url = emberTzData.ajax_url.toString().replace('%%endpoint%%', 'templaza_get_recently_viewed');
			$.post(
				ajax_url,
				function (response) {
		...
		function recentlyViewedProductsCarousel() {
			...
			new Swiper($related.find('.history-products-carousel'), {
				...
				breakpoints: {
					300: {
						slidesPerView: emberTzData.mobile_portrait == '' ? 2 : emberTzData.mobile_portrait,
						slidesPerGroup: emberTzData.mobile_portrait == '' ? 2 : emberTzData.mobile_portrait,
						spaceBetween: 15,
					},
					480: {
						slidesPerView: emberTzData.mobile_landscape == '' ? 3 : emberTzData.mobile_landscape,
						slidesPerGroup: emberTzData.mobile_landscape == '' ? 3 : emberTzData.mobile_landscape,
						spaceBetween: 15,
					},
					768: {
						spaceBetween: 20,
